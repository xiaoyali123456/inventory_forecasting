...
def custom_tags():
    ...
    t4 = ['s3://' + x for x in t3]
    df = spark.read.parquet(*t4)
    return df.groupby('dw_d_id').agg(F.collect_set('tag_type').alias('segments')) \
        .withColumn('segments', concat('segments')).cache()

def process(dt, ct, use_backup=False, npar = 1):
    print('process', dt)
    ...
    wt2 = wt1.join(ct, on='dw_d_id', how='left')
