...
def get_first_match_data_and_increasing_rate(previous_tournament, tag_tournament_1,
    ...
    first_match_num = last_match_data(all_feature_df.where(f"tournament='{previous_tournament}'"), col) + \
                          ((first_match_data(all_feature_df.where(f"tournament='{tag_tournament_2}'"), col) -
                           ...
                           days_bewteen_st_and_et(last_match_data(all_feature_df.where(f"tournament='{tag_tournament_1}'"), 'date'),
                            first_match_data(all_feature_df.where(f"tournament='{tag_tournament_2}'"), 'date'))) \
                          * days_bewteen_st_and_et(last_match_data(all_feature_df.where(f"tournament='{previous_tournament}'"), 'date'),
                            first_match_data(test_df, 'date'))
    num_increasing_rate = (last_match_data(all_feature_df.where(f"tournament='{base_tournament}'"), col)
                            - first_match_data(all_feature_df.where(f"tournament='{base_tournament}'"), col)) / base_tournament_days
...
for test_tournament in tournament_list[1:]:
    ...
    for match_type in test_match_type_list:
        ...
        first_match_sub_num, sub_num_increasing_rate = get_first_match_data_and_increasing_rate(previous_tournament, tag_tournament_1,
                                                                                                tag_tournament_2, base_tournament,
                                                                                                "total_subscribers_number",
                                                                                                base_tournament_days, test_df)
        first_match_free_num, free_num_increasing_rate = get_first_match_data_and_increasing_rate(previous_tournament, tag_tournament_1,
                                                                                                  tag_tournament_2, base_tournament,
                                                                                                  "total_frees_number",
                                                                                                  base_tournament_days, test_df)
        ...
        if version == "baseline":
            ...
            india_match_df = base_tournament_feature_df\
                .where(f'(if_contain_india_team = 1 and tournament_type = "international") or (tournament_type != "international")')\
            ...
            group_match_df = base_tournament_feature_df\
                .where(f'(if_contain_india_team = 0 and tournament_type = "international") or (tournament_type != "international")')\
                .where(f'match_stage = "group"')\
                .groupBy('tag')\
